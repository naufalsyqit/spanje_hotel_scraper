...
def get_departure_options(price_grid_soup):
    ...
    date_options = [{"text": option.text, "value": option["value"]} for option in get_date_options if option["value"] != ""]
    departure_options = [{"name": input.find_next("label").text, "code": input["value"]} for input in get_departure_options if input["value"] != ""]
...
def get_trip_options(price_details_json):
    ...
    price_per_person = price_grid_soup.find("span", class_="price-per-person").get_text().strip()
    ...
    hotel_info["currency"] = decoded_price_per_person.split()[0].encode("latin1").decode("utf-8")
    hotel_info["meal_plan"] = price_grid_soup.find("span", id="pricegridselectValueBoardType").get_text().strip()
